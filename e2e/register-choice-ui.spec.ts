...
test("매물등록: 거래종류 다시 누르면 세 개가 다시 보인다", async ({ page }) => {
  ...
  try {
    ...
    await roomNo.blur();
    await expect(roomNo).toHaveValue("1203호");

    await roomNo.fill("101-101");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("101동 101호");

    await roomNo.fill("101/101");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("101동 101호");

    await roomNo.fill("101 101호");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("101동 101호");

    await roomNo.fill("101동 101");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("101동 101호");

    await roomNo.fill("힐스테이트 101-101");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("힐스테이트 101동 101호");

    await roomNo.fill("101호");
    await roomNo.blur();
    await expect(roomNo).toHaveValue("101호");

    await expect(page.getByLabel("관리비")).toHaveCount(0);
    await page.getByRole("button", { name: "전세", exact: true }).click();
    await expect(page.getByLabel("관리비")).toBeVisible();
